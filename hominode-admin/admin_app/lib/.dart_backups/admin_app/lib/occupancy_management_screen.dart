...
class OccupancyManagementScreen extends StatefulWidget {
  ...
  @override
  State<OccupancyManagementScreen> createState() => _OccupancyManagementScreenState();
}
...
class _OccupancyManagementScreenState extends State<OccupancyManagementScreen> {
  ...
  List<OccupancyUnit> get filteredUnits {
    ...
    if (selectedBuilding != 'All Buildings') {
      filtered = filtered.where((unit) => unit.building == selectedBuilding).toList();
    }
  ...
  Widget _buildSummaryCards() {
    ...
    final occupiedUnits = units.where((u) => u.status == OccupancyStatus.occupied).length;
    final vacantUnits = units.where((u) => u.status == OccupancyStatus.vacant).length;
    final maintenanceUnits = units.where((u) => u.status == OccupancyStatus.maintenance).length;
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 16),
          ...filteredUnits.map((unit) => Padding(
        ...
        children: [
          ...
          if (unit.status == OccupancyStatus.occupied) ...[
            ...
            if (unit.rentAmount != null) ...[
              ...
              Row(
                children: [
                  Icon(Icons.currency_rupee, size: 16, color: const Color(0xFF6B7280)),
                  const SizedBox(width: 8),
                  Text(
          ...
          if (unit.status == OccupancyStatus.maintenance && unit.maintenanceReason != null) ...[
            const SizedBox(height: 12),
            Row(
              children: [
          ...
          if (unit.status == OccupancyStatus.vacant && unit.expectedRent != null) ...[
            const SizedBox(height: 12),
            Row(
              children: [
                Icon(Icons.currency_rupee, size: 16, color: const Color(0xFF6B7280)),
