...
class _ParkingManagementScreenEnhancedState
          ...
          builder: (context, vehiclesSnapshot) {
            ...
            return Padding(
              ...
              child: Row(
                children: [
                  Expanded(
                    child: _buildStatCard('Total Slots', totalSlots.toString(),
                        const Color(0xFF2563EB), Icons.local_parking),
                  ),
                  const SizedBox(width: 12),
                  Expanded(
                    child: _buildStatCard('Occupied', occupiedSlots.toString(),
  ...
  Widget _buildSlotListItem(ParkingSlotModel slot) {
    ...
    final statusColor = isOccupied ? const Color(0xFF16A34A) : const Color(0xFF9CA3AF);
  ...
  void _showAddParkingSlotModal() {
    AddParkingSlotModal.show(context, onSlotAdded: () {
      setState(() {});
    });
  }
  ...
  Future<void> _resolveViolation(String violationId) async {
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Violation resolved')),
        );
      }
    } catch (e) {
      if (mounted) {
