...
class CloudinaryApartmentImagesService {
  ...
  Future<String> uploadImage({
    ...
    try {
      ...
      try {
        ...
        request.fields['public_id'] = 'apartment_${DateTime.now().millisecondsSinceEpoch}';
        ...
        if (response.statusCode == 200) {
          ...
          print('❌ STEP 3 FAILED: Upload failed with status 401 (Unauthorized)');
          print('   Response body: $responseString');
          
          // 401 means preset doesn't exist or isn't unsigned
          String errorMessage = 'Cloudinary error: Upload preset not configured\n\n';
  ...
  Stream<List<ApartmentImageModel>> getImagesForBuilding(String buildingId) {
    try {
      print('🔵 CLOUDINARY APARTMENT IMAGES SERVICE: Fetching images for building - $buildingId');
  ...
  Future<void> deleteImage(String imageId) async {
    try {
      print('🔵 CLOUDINARY APARTMENT IMAGES SERVICE: Deleting image - $imageId');
