    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          _buildResidentRow('Name :', widget.unit.residentName ?? 'Not Available'),
          const SizedBox(height: 16),
  ...
  Future<void> _handleRemoveResident() async {
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text('Are you sure you want to remove ${widget.unit.residentName ?? "this resident"} from ${widget.unit.id}?'),
        actions: [
          TextButton(
  ...
  Future<void> _handleStatusChange(String selectedStatus) async {
    ...
    try {
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Flat ${widget.unit.id} status updated to $selectedStatus'),
            backgroundColor: const Color(0xFF10B981),
            duration: const Duration(seconds: 2),
          ),
        );
