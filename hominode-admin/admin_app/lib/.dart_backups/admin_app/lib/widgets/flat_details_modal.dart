      ...
      await AssignResidentModal.show(
        ...
        loadResidents: widget.userService != null 
            ? () async {
                ...
                final users = await widget.userService!.getAllResidentsWithStatus().first;
                print('🔵 Fetched ${users.length} users from Firestore (all statuses)');
        ...
        onAssign: (request) async {
          ...
          if (widget.userService == null || widget.flatService == null || widget.buildingService == null) {
          ...
          try {
            // Get user details
            final user = await widget.userService!.getUserById(request.residentId);
            if (user == null) {
            ...
            if (mounted) {
              ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(
                  content: Text('${user.name} assigned to ${widget.unit.id} successfully'),
        ...
        onAssignNew: (request) async {
          ...
          try {
            ...
            // Update flat status
            await widget.flatService!.assignResident(
              flatId: widget.unit.docId,  // Use Firestore document ID, not sequential ID
            ...
            if (mounted) {
              ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(
                  content: Text('${request.name} created and assigned to ${widget.unit.id}'),
                  backgroundColor: const Color(0xFF10B981),
                  duration: const Duration(seconds: 3),
                ),
              );
