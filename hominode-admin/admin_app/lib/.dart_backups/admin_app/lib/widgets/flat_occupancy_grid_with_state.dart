...
class FlatOccupancyGridWithState extends StatefulWidget {
  ...
  const FlatOccupancyGridWithState({
    super.key,
  ...
  static Future<void> show(
    ...
    return showGeneralDialog(
      ...
      pageBuilder: (context, animation, secondaryAnimation) {
        return FlatOccupancyGridWithState(
...
class _FlatOccupancyGridWithStateState
  ...
  List<FlatUnit> get _filteredFlats {
    ...
    return allFlats.where((flat) {
      ...
      final matchesSearch = _searchQuery.isEmpty ||
          ...
          (flat.resident?.name.toLowerCase().contains(_searchQuery.toLowerCase()) ??
              false);
  ...
  Widget _buildLegend() {
    ...
    return Wrap(
      ...
      runSpacing: 8,
      children: [
        _buildLegendItem('Occupied', const Color(0xFF10B981), counts['occupied'] ?? 0),
        _buildLegendItem('Vacant', const Color(0xFFD1D5DB), counts['vacant'] ?? 0),
        _buildLegendItem('Maintenance', const Color(0xFFFBBF24), counts['maintenance'] ?? 0),
      ],
    );
  }
  ...
  Widget _buildFlatTile(FlatUnit flat) {
    ...
    final textColor =
        flat.status == FlatStatus.vacant ? const Color(0xFF6B7280) : Colors.white;
