...
class CommunityInviteService {
  ...
  static Map<String, dynamic> communityCreationPayload({
    ...
    if (location != null) {
      payload['location'] = location.toMap();
    }

    return payload;
  }

  Future<TenantConfig> getSelectedCommunity() async {
    final communityId = _adminService.requireCurrentCommunityId();
    try {
      return await TenantRegistryService(
        firestore: _firestore,
      ).getTenantById(communityId);
  ...
  Future<void> createCommunity({
    ...
    // But when supplied, it must be valid.
    if (location != null) {
      if (!location.isValid) {
        throw ArgumentError('Community location is invalid.');
      }

      if (location.formattedAddress.trim().isEmpty) {
        throw ArgumentError('Community address is required.');
      }

      if (location.attendanceRadiusMeters < 25 ||
          location.attendanceRadiusMeters > 5000) {
        throw ArgumentError(
  ...
  Future<TenantConfig> updateCommunityLocation(
    TenantConfig community,
    CommunityLocation location,
  ) async {
    if (!location.isValid || location.formattedAddress.trim().isEmpty) {
      throw ArgumentError('Community location is invalid.');
    }
    try {
      await _functions.httpsCallable('updateCommunityLocation').call({
        'communityId': community.communityId,
        'location': location.toMap(),
      });
      final fresh = await TenantRegistryService(
        firestore: _firestore,
      ).getTenantByIdFromServer(community.communityId);
      AdminTenantContext.instance.refreshTenant(fresh);
      return fresh;
    } on FirebaseFunctionsException catch (error) {
      throw StateError(
        error.message ?? 'Community location could not be updated.',
      );
    }
  }
