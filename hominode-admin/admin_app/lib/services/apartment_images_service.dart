...
class ApartmentImagesService {
  ...
  Future<String> uploadImage({
    ...
    try {
      ...
      try {
        final fileName =
            'apartment_image_${DateTime.now().millisecondsSinceEpoch}.jpg';
        final storagePath = 'apartment_images/$communityId/$adminId/$fileName';
        ...
        print(
          '📤 STEP 3.2: Upload task completed - Bytes transferred: ${uploadTask.bytesTransferred}',
        );
  ...
  Stream<List<ApartmentImageModel>> getImages() {
    try {
      ...
      if (adminId == null) {
        print(
          '⚠️ WARNING: Admin not authenticated, returning local images only',
        );
        // Return local images if not authenticated
        final localImages = _localImages
            .where((img) => img.communityId == communityId)
            .toList();
      ...
      return _firestore
          ...
          .map((snapshot) {
            print(
              '✅ STEP 2 PASSED: Received ${snapshot.docs.length} Firestore images',
            );
            ...
            print(
              '📋 STEP 3a: Combining with ${_localImages.length} local images...',
            );
            final allImages = [
              ...firestoreImages,
              ..._localImages.where((img) => img.communityId == communityId),
            ];
  ...
  Stream<List<ApartmentImageModel>> getImagesForBuilding(String buildingId) {
    try {
      print(
        '🔵 APARTMENT IMAGES SERVICE: Fetching images for building - $buildingId',
      );
          ...
          .map((snapshot) {
            ...
            final localImages = _localImages
                .where(
                  (img) =>
                      img.status == 'active' &&
                      img.buildingIds.contains(buildingId),
                )
                .where((img) => img.communityId == communityId)
                .toList();
